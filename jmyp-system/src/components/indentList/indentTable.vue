<template>
  <div class="table">
    <el-table :data="tableData" border @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="orderId" label="订单编号" width="180" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column prop="price" label="用户账号" width="90" align="center">
        <template slot-scope="scope">
          <p>{{scope.row.test}}</p>
        </template>
      </el-table-column>
      <el-table-column prop="label" label="订单金额" width="100" align="center">
        <template slot-scope="scope">
          <p>{{scope.row.money}}</p>
        </template>
      </el-table-column>
      <el-table-column prop="sort" label="支付方式" width="100" align="center">
        <template slot-scope="scope">
          <p>{{scope.row.payway}}</p>
        </template>
      </el-table-column>
      <el-table-column prop="address" label="订单来源" width="100" align="center">
        <template slot-scope="scope">
          <p>{{scope.row.orderorigin}}</p>
        </template>
      </el-table-column>
      <el-table-column prop="sales" label="订单状态" width="150" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column prop="audit" label="操作" width="200" align="center">
        <template class="flex-button" slot-scope="scope">
          <label class="operation">
            <el-button size="small" @click="Tiao(scope.row.id)">查看订单</el-button>
          </label>
          <label class="operation dangerbutton">
            <el-button size="small" type="danger" @click="Del">删除订单</el-button>
          </label>
        </template>
      </el-table-column>
    </el-table>
